...
D3D12_SHADER_BYTECODE UIShader::CreateVertexShader(ID3DBlob **ppd3dShaderBlob, UINT nIndex)
{
	...
	if (nIndex == Teduri)
		return(CompileShaderFromFile(L"UI.hlsl", "TEDURI_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == hpbar)
		return(CompileShaderFromFile(L"UI.hlsl", "HPBAR_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == MGICON)
		return(CompileShaderFromFile(L"UI.hlsl", "MGIKON_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == BOSSTEDURI)
		return(CompileShaderFromFile(L"UI.hlsl", "TEDURI_BOSS_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == BOSSHP)
		return(CompileShaderFromFile(L"UI.hlsl", "BOSS_HPBAR_VS", "vs_5_1", ppd3dShaderBlob));
}
...
{
	//                  개수
	m_pTexture = new CTexture(5, RESOURCE_TEXTURE2D, 0);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/Image/UI/ui_bg.dds", 0);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/Image/UI/hp_bar.dds", 1);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/Image/UI/face.dds", 2);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/Image/UI/uf_bg_boss.dds", 3);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/Image/UI/boss_hp_bar.dds", 4);
...
void UIShader::updateHP(CPlayer *pPlayer, CGameObject* pMon)
{
	//m_ui.hp = pPlayer->hp;
	//m_ui.bosshp = pMon->hp;
