...
void CGameFramework::BuildObjects()
{
	...
	//m_pScene->m_pPlayer->SetPosition(m_pPlayer->GetPosition());

	m_pCamera = m_pPlayer->GetCamera();
	

	//m_pScene->m_pTerrain = new CHeightMapTerrain
	...
	m_pPlayer->SetPosition(XMFLOAT3(1000.0f, 300.0f, 1000.0f));  // ¿ø·¡ 0,0,0ÀÎµ¥ ÅÍ·¹ÀÎ À§Ä¡ ¸ø¹Ù²ãÁà¼­ ÇÃ·¹ÀÌ¾î À§Ä¡ ¹Ù²Þ 17.12.11
//	m_pPlayer->Rotate(0.0f, 0.0f, 0.0f);
	...
	//		m_pPlayer->m_vBullets[j]->ReleaseUploadBuffers( );
...
void CGameFramework::ProcessInput()
{
	...
	if (!bProcessedByScene)
	{
		...
		if (pKeysBuffer[VK_SPACE] & 0xF0)
		{
			...
			{
				m_pPlayer->Fire();
				//m_pScene->isRenderBullet = true;   //ÇÃ·¹ÀÌ¾î ÁÂÇ¥¶û ¹æÇâ ¹Þ¾Æ¾ßµÇ´Ï±ñ ¾ÀÀ¸·Î ³Ñ°ÜÁÜ 
				//m_pScene->ReBuildObjects(m_pd3dDevice,m_pd3dCommandList);
			}
			//m_pPlayer->isFire = true;isRenderBullet
			m_bisFire = true;
		}
		else
		{	
			m_bisFire = false;
			//m_pScene->isRenderBullet = false;
...
{    
	...
	/*for (int i = 0; i < m_pPlayer->m_vBullets.size(); ++i)
	{
		...
		m_pPlayer->m_vBullets[i]->Render(m_pd3dCommandList, m_pCamera);
		m_pPlayer->m_vBullets[i]->UpdateTransform(NULL);
	}*/
